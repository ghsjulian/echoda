...
const EditBlog = () => {
    ...
    const { isSaving, editBlog, blogs } = useApi();
    const { id } = useParams();
    ...
    const getBlog = id => {
        if (blogs?.length > 0) {
            blogs.filter((blog, item) => {
                if (blog?._id === id) {
                    let temData = blog;
                    temData.blogImage = blog?.blogImage?.url;
                    setData(temData);
                }
            });
        }
    ...
    const handleSubmit = async () => {
        ...
        await editBlog(data,id, showMessage,navigate);
    };
        ...
        <section className="edit-about">
            ...
            <div className="blog-form">
                ...
                <button onClick={handleSubmit}>
                    {isSaving ? "Processing..." : "Update Now"}
                </button>
            </div>
        </section>
    );
